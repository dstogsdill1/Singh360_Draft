...
def main() -> int:
    ...
    checks = {
        ...
        "browseReturnsPathOnly": (not repository_mode) or "selectedPath" in server,
        "browseDoesNotAutoLink": (not repository_mode) or "doc, status = set_workbook_link" not in server[server.find("def workbook_link_pick"):server.find("def workbook_link_sync")],
        "staPicker": "-STA" in (root / "core/workbook_link_manager.py").read_text(encoding="utf-8"),
