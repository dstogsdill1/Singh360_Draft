...
def main() -> int:
    ...
    try:
        ...
        initial_symbol_count = len(standard.get_json()["template"]["symbols"])
        assert initial_symbol_count >= 13
        ...
        assert updated_standard.get_json()["total"] == initial_symbol_count + 1
        ...
        sid = session["id"]
        assert len(session["template"]["symbols"]) == initial_symbol_count + 1
