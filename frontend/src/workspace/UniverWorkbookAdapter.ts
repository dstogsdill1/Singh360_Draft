...
export function fromUniverWorkbook(snapshot: IWorkbookData, previous: WorkbookDocument): WorkbookDocument {
  const sheets = snapshot.sheetOrder.map((id) => {
    ...
    Object.entries(sheet.rowData || {}).forEach(([row, data]) => {
      if (data?.h) rowHeights[String(Number(row) + 1)] = data.h;
    });
    const columnWidths: Record<string, number> = {};
    Object.entries(sheet.columnData || {}).forEach(([column, data]) => {
      if (data?.w) columnWidths[letters(Number(column))] = data.w / 7;
    });
    return {
      id,
      name: sheet.name || id,
      ...
      merges: (sheet.mergeData || []).map((range) =>
        `${letters(range.startColumn)}${range.startRow + 1}:${letters(range.endColumn)}${range.endRow + 1}`),
      rowHeights,
      columnWidths,
      archived: Boolean(sheet.hidden),
      tabColor: (sheet as { tabColor?: string }).tabColor
        || previous.sheets.find((item) => item.id === id)?.tabColor,
    };
  });
